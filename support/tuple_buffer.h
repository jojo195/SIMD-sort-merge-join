...
#define CHAINEDBUFF_NUMTUPLESPERBUF TRANSFER_TUPLE_NUM
...
/**
 * implement output_relation as two chained buffer
 * one buffer for R relation, another for S
 * one R tuple sequentially mapped S tuple
*/
struct output_relation_t {
    chainedtuplebuffer_t* output_R;
    chainedtuplebuffer_t* output_S;
};
...
static output_relation_t* output_relation_init(void)
{
    output_relation_t rt;
    rt.output_R=chainedtuplebuffer_init();
    rt.output_S=chainedtuplebuffer_init();
    return &rt;
}

static void output_relation_free(output_relation_t* a)
{
    chainedtuplebuffer_free(a->output_R);
    chainedtuplebuffer_free(a->output_S);
}

static void output_relation_add(output_relation_t* rel, tuple_t* r, tuple_t* s)
{
    tuple_t* target_pos=cb_next_writepos(rel->output_R);
    memcpy(target_pos, r, sizeof(tuple_t));
    target_pos=cb_next_writepos(rel->output_S);
    memcpy(target_pos, s, sizeof(tuple_t));
}
